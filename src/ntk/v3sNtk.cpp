...
const V3NetId
V3SNTk::createNet(const uint32_t& width) {
   // Validation Check
   if (width != 1) {
...
void
V3SNTk::createModule(V3SNTkModule* const module) {
   assert (module); assert (module->getNtkRef());
   _ntkModule.push_back(module);
}

void
V3SNTk::createInput(const V3NetId& id) {
...
void
V3SNTk::replaceFanin(const V3RepIdHash& repIdHash) {
   ...
   for (V3NetId id = V3NetId::makeNetId(1); id.id < _inputData.size(); ++id.id) {
      type = getGateType(id);
      if (V3_MODULE == type) {
         V3SNTkModule* const module = getModule(_inputData[id.id][0].value); assert (module);
...
const bool
V3SNTk::reportMultipleDrivenNet(const V3GateType& type, const V3NetId& id) const {
...
const bool
V3SNTk::reportUnexpectedFaninSize(const V3GateType& type, const V3NetId& id) const {
...
const bool
V3SNTk::reportNetWidthInconsistency(const V3NetId& id1, const V3NetId& id2, const string& s) const {
...
const bool
V3SNTk::reportUnexpectedNetWidth(const V3NetId& id, const uint32_t& width, const string& s) const {
...
V3BvNtk::V3BvNtk() : V3SNTk() {
   _netWidth.clear();
}

V3BvNtk::V3BvNtk(const V3BvNtk& ntk) : V3SNTk(ntk) {

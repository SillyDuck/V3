...
V3SvrBase* const allocSolver(const V3SolverType& type, const V3Ntk* const ntk, const bool& freeBound ) {
   ...
   switch (type) {
      ...
      case V3S_SVR_MINISAT    : return new V3SSvrMiniSat(ntk, freeBound);
      default                 : return 0;
   }
}
...
V3SvrBase* const referenceSolver(V3SvrBase* const solver) {
   ...
   else if (dynamic_cast<V3SSvrMiniSat*>(solver)) return new V3SSvrMiniSat(*(dynamic_cast<V3SSvrMiniSat*>(solver)));
   else return 0;
}
